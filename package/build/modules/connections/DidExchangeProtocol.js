...
let DidExchangeProtocol = class DidExchangeProtocol {
    ...
    async createRequest(agentContext, outOfBandRecord, params) {
        ...
        const didDocumentCurrent = (await agentContext.dependencyManager.resolve(dids_1.DidsApi).getCreatedDids({ method: 'web' }))[0].didDocument;
        const didlong = didDocumentCurrent.id;
        const message = new messages_1.DidExchangeRequestMessage({ label, parentThreadId, did: didDocument.id, goal, goalCode, didlong });
        ...
        if ((0, dids_1.isValidPeerDid)(didDocument.id) && (0, dids_1.getNumAlgoFromPeerDid)(didDocument.id) === dids_1.PeerDidNumAlgo.GenesisDoc) {
            const keyForSignature = [];
            if (didDocumentCurrent)
                keyForSignature.push(didDocumentCurrent.verificationMethod[0].publicKeyBase58);
            const didDocAttach = await this.createSignedAttachment(agentContext, didDocument.toJSON(), keyForSignature.length > 0 ? keyForSignature : didDocument.recipientKeys.map((key) => key.publicKeyBase58));
    ...
    async createResponse(agentContext, connectionRecord, outOfBandRecord, routing) {
        ...
        if (numAlgo === dids_1.PeerDidNumAlgo.GenesisDoc) {
            message.didDoc = await this.createSignedAttachment(agentContext, didDocument.toJSON(), keyForSignature.length > 0 ? keyForSignature : Array.from(// if key:web defined use it
            new Set(services
            ...
            message.didRotate = await this.createSignedAttachment(agentContext, didDocument.id, keyForSignature.length > 0 ? keyForSignature : Array.from(// if key:web defined use it
            new Set(services
                .map((s) => s.recipientKeys)
                .reduce((acc, curr) => acc.concat(curr), [])
                .map((key) => key.publicKeyBase58))));
    ...
    async extractAttachedDidDocument(agentContext, message, invitationKeysBase58 = []) {
        ...
        /* Change this check, let us stuck here only if the key used for signing is different from the key stored in DIDDoc
        ...
        let base58DidLong = didDocumentKeysBase58[0];
        if (message instanceof messages_1.DidExchangeRequestMessage) {
            const result = await agentContext.dependencyManager.resolve(dids_1.DidsApi).resolveDidDocument(message.didlong);
            base58DidLong = result.verificationMethod[0].publicKeyBase58;
            if (!isValid || !signerKeys.every((key) => base58DidLong == key.publicKeyBase58)) {
                const problemCode = errors_1.DidExchangeProblemReportReason.RequestNotAccepted;
            ...
            if (!isValid || !signerKeys.every((key) => didDocumentKeysBase58 === null || didDocumentKeysBase58 === void 0 ? void 0 : didDocumentKeysBase58.includes(key.publicKeyBase58))) {
                const problemCode = errors_1.DidExchangeProblemReportReason.ResponseNotAccepted;
                throw new errors_1.DidExchangeProblemReportError('DID Document signature is invalid.', { problemCode });
            }
        }
        return didDocument;
