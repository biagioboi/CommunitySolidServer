...
class ProofFormatCoordinator {
     ...
     *
     */
    async createWrappedRequest(agentContext, { proofFormats, formatServices, proofRecord, comment, goalCode, goal, presentMultiple, willConfirm, requestWrapper }) {
        const didCommMessageRepository = agentContext.dependencyManager.resolve(didcomm_1.DidCommMessageRepository);
        // create message. there are two arrays in each message, one for formats the other for attachments
        const formats = [];
        const requestAttachments = [];
        for (const formatService of formatServices) {
            const { format, attachment } = await formatService.createWrappedRequest(agentContext, {
                requestWrapper,
                proofFormats,
                proofRecord,
            });
    ...
    async processRequest(agentContext, { proofRecord, message, formatServices, }) {
        const didCommMessageRepository = agentContext.dependencyManager.resolve(didcomm_1.DidCommMessageRepository);
        for (const formatService of formatServices) {
            let attachment = this.getAttachmentForService(formatService, message.formats, message.requestAttachments);
            if (attachment.data.json.wrappedVPR !== undefined) {
                attachment.data.json = attachment.data.json.wrappedVPR;
            }
