...
class DifPresentationExchangeProofFormatService {
    ...
    async createRequest(agentContext, { attachmentId, proofFormats }) {
        ...
        const { presentationDefinition, options, signPresentationRequest, signACPContext } = presentationExchangeFormat;
        ps.validatePresentationDefinition(presentationDefinition);
        const format = new models_1.ProofFormatSpec({
            format: PRESENTATION_EXCHANGE_PRESENTATION_REQUEST,
            attachmentId,
        });
        let presentation_signed = await ps.createPresentationRequest(agentContext, presentationDefinition, options, signPresentationRequest, signACPContext, presentationExchangeFormat['@context'] || [], presentationExchangeFormat.type || []);
        const attachment = this.getFormatData(presentation_signed, format.attachmentId);
    ...
    async processPresentation(agentContext, { requestAttachment, attachment }) {
        ...
        let partialRequestAttachment = requestAttachment;
        if (partialRequestAttachment.data.json.type[0] === "VerifiablePresentationRequestWrapper") {
            partialRequestAttachment.data.json = partialRequestAttachment.data.json.wrappedVPR;
            requestAttachment = partialRequestAttachment;
        }
    ...
    async createWrappedRequest(agentContext, { attachmentId, proofFormats, requestWrapper }) {
        const format = new models_1.ProofFormatSpec({
            format: PRESENTATION_EXCHANGE_PRESENTATION_REQUEST,
            attachmentId,
        });
        const attachment = this.getFormatData(requestWrapper, format.attachmentId);
        return { attachment, format };
    }
}
