...
class JsonLdCredentialFormatService {
    ...
    async createProposal(agentContext, { credentialFormats }) {
        ...
        if (!jsonLdFormat) {
            throw new error_1.CredoError('Missing jsonld payload in createProposal');
        }
    ...
    async processProposal(agentContext, { attachment }) {
        ...
        if (!credProposalJson) {
            throw new error_1.CredoError('Missing jsonld credential proposal data payload');
        }
    ...
    async createOffer(agentContext, { credentialFormats, attachmentId }) {
        ...
        if (!jsonLdFormat) {
            throw new error_1.CredoError('Missing jsonld payload in createOffer');
        }
        // validate
        JsonTransformer_1.JsonTransformer.fromJSON(jsonLdFormat.credential, JsonLdCredentialDetail_1.JsonLdCredentialDetail);
        const attachment = this.getFormatData(jsonLdFormat, format.attachmentId);
    ...
    async processOffer(agentContext, { attachment }) {
        ...
        if (!credentialOfferJson) {
            throw new error_1.CredoError('Missing jsonld credential offer data payload');
        }
    ...
    async createRequest(agentContext, { credentialFormats }) {
        ...
        if (!jsonLdFormat) {
            throw new error_1.CredoError('Missing jsonld payload in createRequest');
        }
        // this does the validation
        JsonTransformer_1.JsonTransformer.fromJSON(jsonLdFormat.credential, JsonLdCredentialDetail_1.JsonLdCredentialDetail);
        const attachment = this.getFormatData(jsonLdFormat, format.attachmentId);
    ...
    async processRequest(agentContext, { attachment }) {
        ...
        if (!requestJson) {
            throw new error_1.CredoError('Missing jsonld credential request data payload');
        }
        // validate
    ...
    async acceptRequest(agentContext, { credentialFormats, attachmentId, requestAttachment }) {
        ...
        if (!verificationMethod) {
            throw new error_1.CredoError('Missing verification method in credential data');
        }
        const format = new CredentialFormatSpec_1.CredentialFormatSpec({
            attachmentId,
            format: JSONLD_VC,
        ...
        if (foundFields.length > 0) {
            throw new error_1.CredoError(`Some fields are not currently supported in credential options: ${foundFields.join(', ')}`);
        }
    ...
    async deriveVerificationMethod(agentContext, credentialAsJson, credentialRequest) {
        ...
        if (!keyType || keyType.length === 0) {
            throw new error_1.CredoError(`No Key Type found for proofType ${proofType}`);
        }
        const verificationMethod = await (0, DidDocument_1.findVerificationMethodByKeyType)(keyType[0], issuerDidDocument);
        if (!verificationMethod) {
            throw new error_1.CredoError(`Missing verification method for key type ${keyType}`);
        }
    ...
    async processCredential(agentContext, { credentialRecord, attachment, requestAttachment }) {
        ...
        if (result && !result.isValid) {
            ...
            throw new error_1.CredoError(`Failed to validatecsd credential, error = ${result.error}`);
        }
    ...
    verifyReceivedCredentialMatchesRequest(credential, request) {
        ...
        if (Array.isArray(credential.proof)) {
            throw new error_1.CredoError('Credential proof arrays are not supported');
        }
        if (request.options.created && credential.proof.created !== request.options.created) {
            throw new error_1.CredoError('Received credential proof created does not match created from credential request');
        }
        if (credential.proof.domain !== request.options.domain) {
            throw new error_1.CredoError('Received credential proof domain does not match domain from credential request');
        }
        if (credential.proof.challenge !== request.options.challenge) {
            throw new error_1.CredoError('Received credential proof challenge does not match challenge from credential request');
        }
        if (credential.proof.type !== request.options.proofType) {
            throw new error_1.CredoError('Received credential proof type does not match proof type from credential request');
        }
        if (credential.proof.proofPurpose !== request.options.proofPurpose) {
            throw new error_1.CredoError('Received credential proof purpose does not match proof purpose from credential request');
        }
        ...
        if (!(0, utils_1.areObjectsEqual)(jsonCredential, request.credential)) {
            throw new error_1.CredoError('Received credential does not match credential request');
        }
