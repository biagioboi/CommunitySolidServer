...
const W3cPresentationRequest_1 = require("../vc/models/presentation/W3cPresentationRequest");
/**
...
let DifPresentationExchangeService = class DifPresentationExchangeService {
    constructor(w3cCredentialService) {
        ...
        this.pex = new pex_1.PEX({ hasher: utils_1.Hasher.hash });
    }
    ...
    async createPresentation(agentContext, options) {
            ...
            const verifiablePresentationResult = await this.pex.verifiablePresentationFrom(presentationDefinitionForSubject, credentialsForPresentation, this.getPresentationSignCallback(agentContext, presentationToCreate), {
                basePresentationPayload: {
                    '@context': [
                        ...
                        'https://bboi.solidcommunity.net/public/schemas/2024/protocol.jsonld'
                    ]
                },
                proofOptions: {
                    challenge,
        ...
        return {
            verifiablePresentations: verifiablePresentationResultsWithFormat.map((resultWithFormat) => (0, utils_2.getVerifiablePresentationFromEncoded)(agentContext, resultWithFormat.verifiablePresentationResult.verifiablePresentation)),
    ...
    async createPresentationRequest(agentContext, presentationDefinition, options, signPresentationRequest, signACPContext, context, type) {
        if (signACPContext) {
            let returned = await this.getACPContextSign(agentContext, presentationDefinition, options, context, type);
            presentationDefinition.requestACP = returned;
        }
        if (signPresentationRequest) {
            return await this.getPresentationRequestSign(agentContext, presentationDefinition, options, context, type);
        }
        else {
            return new W3cPresentationRequest_1.W3cPresentationRequest({
                context: context,
                type: type,
    ...
    async getPresentationRequestSign(agentContext, presentationExchangeRequest, options, context, type) {
        const didRecord = await agentContext.dependencyManager.resolve(dids_1.DidsApi).getCreatedDids({ method: 'web' });
        const ourDID = didRecord[0].didDocument.id;
        const verificationMethod = await this.getVerificationMethodForSubjectId(agentContext, ourDID || "");
        let presentationRequestFull = {
            '@context': context,
            type: type,
            presentation_definition: presentationExchangeRequest,
            options: {
        ...
        if (!verificationMethod) {
            throw new DifPresentationExchangeError_1.DifPresentationExchangeError(`Cannot create presentation request for credentials without a valid ACP owner`);
        }
        return await this.w3cCredentialService.signPresentationRequest(agentContext, {
            format: vc_1.ClaimFormat.LdpVc,
            proofType: this.getProofTypeForLdpVc(agentContext, presentationExchangeRequest, verificationMethod),
            verificationMethod: verificationMethod.id,
            presentationRequest: presentationRequestFull,
        });
    }
    async getACPContextSign(agentContext, presentationExchangeRequest, options, context, type) {
        const didRecord = await agentContext.dependencyManager.resolve(dids_1.DidsApi).getCreatedDids({ method: 'web' });
        const ourDID = didRecord[0].didDocument.id;
        const verificationMethod = await this.getVerificationMethodForSubjectId(agentContext, ourDID || "");
        if (presentationExchangeRequest.requestACP === undefined) {
            throw new error_1.CredoError("Unable to process request with ACP Context undefined");
        }
        let definedACPContext = {
            '@context': ["https://bboi.solidcommunity.net/public/schemas/2024/presexchange.jsonld"],
            type: ["NonRepudiableACPContext"],
            definedACPContext: presentationExchangeRequest.requestACP,
        };
        if (!verificationMethod) {
            throw new DifPresentationExchangeError_1.DifPresentationExchangeError(`Cannot create presentation request for credentials without a valid ACP owner`);
        }
        return await this.w3cCredentialService.signACPContext(agentContext, {
            format: vc_1.ClaimFormat.LdpVc,
            proofType: this.getProofTypeForLdpVc(agentContext, presentationExchangeRequest, verificationMethod),
            proofPurpose: 'authentication',
            verificationMethod: verificationMethod.id,
            acpPolicy: definedACPContext,
            challenge: options === null || options === void 0 ? void 0 : options.challenge,
            domain: options === null || options === void 0 ? void 0 : options.domain,
        });
    }
    ...
    async queryCredentialForPresentationDefinition(agentContext, presentationDefinition) {
        ...
        const sdJwtVcApi = this.getSdJwtVcApi(agentContext);
        const sdJwtVcRecords = sdJwtVcQuery.length > 0
