...
const W3cPresentationRequestWrapper_1 = require("../models/presentation/W3cPresentationRequestWrapper");
/**
...
let W3cJsonLdCredentialService = class W3cJsonLdCredentialService {
    ...
    async verifyCredential(agentContext, options) {
        ...
        try {
            ...
            const { verified: isValid } = result, remainingResult = __rest(result, ["verified"]);
            if (!isValid) {
                agentContext.config.logger.debug(`Credential verification failed: ${(_b = result.error) === null || _b === void 0 ? void 0 : _b.message}`, {
                    stack: (_c = result.error) === null || _c === void 0 ? void 0 : _c.stack,
                });
            }
     ...
     * @returns the signed presentation
     */
    async signWrappedPresentation(agentContext, options) {
        // create keyPair
        const WalletKeyPair = (0, WalletKeyPair_1.createWalletKeyPairClass)(agentContext.wallet);
        const suiteInfo = this.signatureSuiteRegistry.getByProofType(options.proofType);
        if (!suiteInfo) {
        ...
        return utils_1.JsonTransformer.fromJSON(result, models_1.W3cWrappedVerifiablePresentation);
    }
    /**
     ...
     * @returns the signed presentation
     */
    async signPresentationRequest(agentContext, options) {
        // create keyPair
        const WalletKeyPair = (0, WalletKeyPair_1.createWalletKeyPairClass)(agentContext.wallet);
        const signingKey = await this.getPublicKeyFromVerificationMethod(agentContext, options.verificationMethod);
        const suiteInfo = this.signatureSuiteRegistry.getByProofType(options.proofType);
        if (!suiteInfo) {
            throw new error_1.CredoError(`The requested proofType Ed25519Signature2018 is not supported`);
        }
        if (!suiteInfo.keyTypes.includes(signingKey.keyType)) {
            throw new error_1.CredoError('The key type of the verification method does not match the suite');
        }
        ...
        const suite = new SuiteClass({
            ...
            useNativeCanonize: false,
            date: (0, util_1.w3cDate)(),
        });
        const result = await vc_1.default.signPresentationRequest({
            presentation: utils_1.JsonTransformer.toJSON(options.presentationRequest),
            suite: suite,
            purpose: options.proofPurpose,
            documentLoader: this.w3cCredentialsModuleConfig.documentLoader(agentContext),
        });
        return new models_2.W3cPresentationRequest(result);
    }
    /**
     * Signs a presentation request
     *
     ...
     * @returns the signed presentation
     */
    async signACPContext(agentContext, options) {
        // create keyPair
        const WalletKeyPair = (0, WalletKeyPair_1.createWalletKeyPairClass)(agentContext.wallet);
        const suiteInfo = this.signatureSuiteRegistry.getByProofType(options.proofType);
        if (!suiteInfo) {
        ...
        const result = await vc_1.default.signPresentation({
            presentation: utils_1.JsonTransformer.toJSON(options.acpPolicy),
            suite: suite,
            challenge: options.challenge,
            domain: options.domain,
            documentLoader: this.w3cCredentialsModuleConfig.documentLoader(agentContext),
        ...
        return utils_1.JsonTransformer.fromJSON(result, models_2.W3cPresentationRequest);
    }
    /**
     * Signs a presentation request
     *
     ...
     * @returns the signed presentation
     */
    async signWrappedPresentationRequest(agentContext, options) {
        // create keyPair
        const WalletKeyPair = (0, WalletKeyPair_1.createWalletKeyPairClass)(agentContext.wallet);
        const signingKey = await this.getPublicKeyFromVerificationMethod(agentContext, options.verificationMethod);
        const suiteInfo = this.signatureSuiteRegistry.getByProofType(options.proofType);
        ...
        const result = await vc_1.default.signPresentationRequest({
            presentation: utils_1.JsonTransformer.toJSON(options.wrappedVPR),
            suite: suite,
            purpose: options.proofPurpose,
            documentLoader: this.w3cCredentialsModuleConfig.documentLoader(agentContext),
        });
        return new W3cPresentationRequestWrapper_1.W3cPresentationRequestWrapper(result);
    }
    /**
     ...
     * @returns the verification result
     */
    async verifyWrappedPresentation(agentContext, options) {
        try {
            // create keyPair
            const WalletKeyPair = (0, WalletKeyPair_1.createWalletKeyPairClass)(agentContext.wallet);
            let proofs = options.wrappedPresentation.proof;
            if (!Array.isArray(proofs)) {
                proofs = [proofs];
            }
            const presentationSuites = proofs.map((proof) => {
            ...
            const verifyOptions = {
                presentation: utils_1.JsonTransformer.toJSON(options.wrappedPresentation),
                suite: presentationSuites,
                challenge: options.challenge,
                domain: options.domain,
                documentLoader: this.w3cCredentialsModuleConfig.documentLoader(agentContext),
            };
